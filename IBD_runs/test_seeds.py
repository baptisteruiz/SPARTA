...
def test_seeding_run_sparta_classification(seed_init = 0):
    ...
    functional_profile_filepath = 'SoFA_table_abundance_IBD.csv'
    label_filepath = 'Label_abundance_IBD.csv'
    output_folder = 'tests_sets_from_paper\\' + 'IBD-output_folder' + "_seed" + str(seed_init)

    run_nb = 3
    nb_iterations = 5
    
    run_sparta_classification(functional_profile_filepath, label_filepath, output_folder, run_nb, nb_iterations, classifiers=20, reference_test_sets_filepath="Test_sets_abundance_IBD.csv", seed_init = seed_init)
...
# nb_runs = 10
# seeds = random.sample(range(1000),nb_runs)

# for i in range(nb_runs):
#     test_seeding_run_sparta_classification(seeds[i])
